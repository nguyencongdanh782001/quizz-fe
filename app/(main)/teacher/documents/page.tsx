  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-primary text-white text-sm font-medium hover:bg-primary/90 transition-colors">
            ...
            {mockDocuments.map((doc, i) => (
              ...
              >
                <td className="px-5 py-4">
                  <p className="font-medium text-on-surface text-sm line-clamp-1">{doc.title}</p>
                  <p className="text-xs text-muted-foreground mt-0.5">{doc.type.toUpperCase()}</p>
                </td>
                <td className="px-5 py-4 text-sm text-muted-foreground">{doc.subject}</td>
                <td className="px-5 py-4 text-sm text-muted-foreground">Lớp {doc.grade}</td>
                <td className="px-5 py-4">
