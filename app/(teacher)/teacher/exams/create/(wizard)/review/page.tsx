  ...
  return (
    <div className="space-y-5 max-w-3xl">
      <div className="bg-surface-container-lowest rounded-xl p-5 space-y-4">
        <h2 className="font-display font-semibold text-lg text-on-surface">Xem lại bài thi</h2>

        {/* Basic info */}
        <div className="space-y-2">
          <h3 className="font-semibold text-sm text-muted-foreground uppercase">Thông tin cơ bản</h3>
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div className="bg-surface rounded-xl p-3">
              <span className="text-muted-foreground">Tiêu đề</span>
              <p className="font-medium text-on-surface mt-0.5">{title || '—'}</p>
            </div>
            <div className="bg-surface rounded-xl p-3">
              <span className="text-muted-foreground">Môn học</span>
              <p className="font-medium text-on-surface mt-0.5">{subject || '—'}</p>
            </div>
            <div className="bg-surface rounded-xl p-3">
              <span className="text-muted-foreground">Khối lớp</span>
              <p className="font-medium text-on-surface mt-0.5">{grade ? `Lớp ${grade}` : '—'}</p>
            </div>
            <div className="bg-surface rounded-xl p-3">
              <span className="text-muted-foreground">Mức độ</span>
              <p className="font-medium text-on-surface mt-0.5 capitalize">{difficulty || '—'}</p>
            </div>
          </div>
          {description && (
            <div className="bg-surface rounded-xl p-3">
        ...
        {/* Questions */}
        <div className="space-y-2">
          <h3 className="font-semibold text-sm text-muted-foreground uppercase">Câu hỏi ({questions.length})</h3>
          <div className="space-y-2">
            {questions.map((q, i) => (
              <div key={q.id} className="bg-surface rounded-xl p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className="text-on-surface text-sm font-medium">{q.text || '(Chưa nhập)'}</p>
                    {q.type !== 'text' && (
                        ...
                        {q.options.map((opt, oi) => (
                          <div key={opt.id} className="flex items-center gap-2 text-sm">
                            <div className={cn(
                              ...
                              opt.isCorrect ? 'bg-primary border-primary' : 'border-outline'
                            )}>
                              {opt.isCorrect && <Check className="w-3 h-3 text-white" />}
                            </div>
                            <span className={opt.isCorrect ? 'text-on-surface' : 'text-muted-foreground'}>
                    ...
                    {q.type === 'text' && (
                      <p className="text-xs text-muted-foreground mt-1">Tự luận</p>
                    )}
                  </div>
                </div>
              </div>
        ...
        {/* Settings */}
        <div className="space-y-2">
          <h3 className="font-semibold text-sm text-muted-foreground uppercase">Cài đặt</h3>
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div className="bg-surface rounded-xl p-3">
              <span className="text-muted-foreground">Thời gian</span>
              <p className="font-medium text-on-surface mt-0.5">{duration} phút</p>
            </div>
            <div className="bg-surface rounded-xl p-3">
              <span className="text-muted-foreground">Điểm đạt</span>
              <p className="font-medium text-on-surface mt-0.5">{passingScore}%</p>
            </div>
            <div className="bg-surface rounded-xl p-3">
            ...
            <div className="bg-surface rounded-xl p-3">
              ...
              <p className="font-medium text-on-surface mt-0.5">{shuffleOptions ? 'Có' : 'Không'}</p>
            </div>
          </div>
        </div>
      </div>
